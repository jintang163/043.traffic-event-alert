...
CREATE TABLE IF NOT EXISTS video_quality_record (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    record_no VARCHAR(64) NOT NULL UNIQUE COMMENT '检测记录编号',
    camera_id BIGINT NOT NULL COMMENT '摄像头ID',
    camera_name VARCHAR(128) COMMENT '摄像头名称',
    ...
    detection_time DATETIME NOT NULL COMMENT '检测时间',
    frame_url VARCHAR(512) COMMENT '检测帧截图URL',
    brightness DECIMAL(8,4) COMMENT '平均亮度 0-255',
    brightness_level INT DEFAULT 1 COMMENT '亮度等级 1-正常 2-偏暗 3-偏亮 4-黑屏',
    contrast DECIMAL(8,4) COMMENT '对比度值',
    contrast_level INT DEFAULT 1 COMMENT '对比度等级 1-正常 2-偏低 3-偏高',
    blur_score DECIMAL(6,4) COMMENT '模糊度评分 0-1 越高越清晰',
    blur_level INT DEFAULT 1 COMMENT '模糊等级 1-清晰 2-轻微模糊 3-严重模糊',
    occlusion_ratio DECIMAL(5,2) DEFAULT 0.00 COMMENT '遮挡率 %',
    occlusion_level INT DEFAULT 1 COMMENT '遮挡等级 1-正常 2-轻微遮挡 3-严重遮挡',
    occlusion_regions TEXT COMMENT '遮挡区域JSON [{x,y,w,h},...]',
    is_black_screen INT DEFAULT 0 COMMENT '是否黑屏 0-否 1-是',
    is_frozen INT DEFAULT 0 COMMENT '是否冻结 0-否 1-是',
    freeze_duration INT DEFAULT 0 COMMENT '冻结持续秒数',
    frame_change_rate DECIMAL(6,4) DEFAULT 1.0000 COMMENT '帧变化率 0-1',
    noise_level DECIMAL(8,4) COMMENT '噪声水平',
    color_cast_level INT DEFAULT 1 COMMENT '偏色等级 1-正常 2-轻微偏色 3-严重偏色',
    overall_score DECIMAL(5,2) DEFAULT 100.00 COMMENT '综合质量评分 0-100',
    quality_level INT DEFAULT 1 COMMENT '综合质量等级 1-优 2-良 3-中 4-差 5-严重异常',
    is_abnormal INT DEFAULT 0 COMMENT '是否异常 0-正常 1-异常',
    abnormal_types VARCHAR(256) COMMENT '异常类型列表 逗号分隔: BLACK_SCREEN/BLUR/OCCLUSION/FREEZE/LOW_BRIGHTNESS/HIGH_BRIGHTNESS/LOW_CONTRAST/COLOR_CAST',
    alert_triggered INT DEFAULT 0 COMMENT '是否已触发告警 0-否 1-是',
    alert_event_id BIGINT COMMENT '关联告警事件ID',
    source_node_code VARCHAR(64) COMMENT '来源边缘节点编码',
    ...
    INDEX idx_alert_triggered (alert_triggered)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='视频质量检测记录表';

CREATE TABLE IF NOT EXISTS video_health_diagnosis (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    diagnosis_no VARCHAR(64) NOT NULL UNIQUE COMMENT '诊断编号',
    camera_id BIGINT NOT NULL COMMENT '摄像头ID',
    camera_name VARCHAR(128) COMMENT '摄像头名称',
    camera_code VARCHAR(64) COMMENT '摄像头编码',
    road_name VARCHAR(128) COMMENT '路段名称',
    longitude DECIMAL(10,6) COMMENT '经度',
    latitude DECIMAL(10,6) COMMENT '纬度',
    diagnosis_date DATE NOT NULL COMMENT '诊断日期(日/周/月起始日期)',
    period_type VARCHAR(16) DEFAULT 'DAILY' COMMENT '统计周期: DAILY/WEEKLY/MONTHLY',
    total_detection_count INT DEFAULT 0 COMMENT '检测总次数',
    abnormal_count INT DEFAULT 0 COMMENT '异常次数',
    normal_rate DECIMAL(5,2) DEFAULT 100.00 COMMENT '正常率 %',
    avg_brightness DECIMAL(8,4) COMMENT '平均亮度',
    avg_contrast DECIMAL(8,4) COMMENT '平均对比度',
    avg_blur_score DECIMAL(6,4) COMMENT '平均清晰度评分',
    avg_occlusion_ratio DECIMAL(5,2) DEFAULT 0.00 COMMENT '平均遮挡率 %',
    avg_overall_score DECIMAL(5,2) DEFAULT 100.00 COMMENT '平均综合评分',
    min_overall_score DECIMAL(5,2) DEFAULT 100.00 COMMENT '最低综合评分',
    max_overall_score DECIMAL(5,2) DEFAULT 100.00 COMMENT '最高综合评分',
    health_score DECIMAL(5,2) DEFAULT 100.00 COMMENT '健康度评分 0-100',
    health_level INT DEFAULT 1 COMMENT '健康等级 1-健康 2-亚健康 3-异常 4-严重异常 5-故障',
    health_level_label VARCHAR(32) COMMENT '健康等级标签',
    black_screen_count INT DEFAULT 0 COMMENT '黑屏次数',
    black_screen_duration INT DEFAULT 0 COMMENT '黑屏累计秒数',
    blur_count INT DEFAULT 0 COMMENT '模糊异常次数',
    occlusion_count INT DEFAULT 0 COMMENT '遮挡异常次数',
    freeze_count INT DEFAULT 0 COMMENT '冻结异常次数',
    freeze_duration INT DEFAULT 0 COMMENT '冻结累计秒数',
    low_brightness_count INT DEFAULT 0 COMMENT '亮度偏低次数',
    high_brightness_count INT DEFAULT 0 COMMENT '亮度偏高次数',
    low_contrast_count INT DEFAULT 0 COMMENT '对比度偏低次数',
    color_cast_count INT DEFAULT 0 COMMENT '偏色异常次数',
    alert_count INT DEFAULT 0 COMMENT '触发告警次数',
    work_order_count INT DEFAULT 0 COMMENT '生成工单数量',
    online_rate DECIMAL(5,2) DEFAULT 100.00 COMMENT '在线率 %',
    uptime_seconds BIGINT DEFAULT 0 COMMENT '正常运行秒数',
    downtime_seconds BIGINT DEFAULT 0 COMMENT '异常停机秒数',
    maintenance_status INT DEFAULT 0 COMMENT '维护状态 0-无需维护 1-建议维护 2-待维修 3-维修中',
    last_maintenance_time DATETIME COMMENT '上次维护时间',
    next_maintenance_suggest DATETIME COMMENT '建议下次维护时间',
    diagnosis_detail TEXT COMMENT '诊断详情JSON',
    recommendation VARCHAR(1024) COMMENT '维护建议',
    status INT DEFAULT 1 COMMENT '状态 0-无效 1-有效',
    create_time DATETIME,
    update_time DATETIME,
    deleted INT DEFAULT 0,
    INDEX idx_diagnosis_no (diagnosis_no),
    INDEX idx_camera_id (camera_id),
    ...
    UNIQUE KEY uk_cam_date_period (camera_id, diagnosis_date, period_type)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='摄像头健康度诊断汇总表';

INSERT INTO video_quality_record (record_no, camera_id, camera_name, camera_code,
    detection_time, brightness, brightness_level, contrast, contrast_level,
    blur_score, blur_level, occlusion_ratio, occlusion_level,
    is_black_screen, is_frozen, overall_score, quality_level, is_abnormal,
    abnormal_types, alert_triggered, description, create_time) VALUES
('VQR202506200001', 1, '京港澳高速K100+500北', 'CAM001',
    '2025-06-20 08:00:00', 128.5000, 1, 65.2000, 1, 0.9200, 1, 0.50, 1,
    0, 0, 95.50, 1, 0, NULL, 0, '画面质量正常', NOW()),
('VQR202506200002', 2, '京港澳高速K100+500南', 'CAM002',
    '2025-06-20 08:00:00', 115.8000, 1, 58.3000, 1, 0.8800, 1, 2.30, 1,
    0, 0, 92.80, 2, 0, NULL, 0, '轻微薄雾，整体正常', NOW()),
('VQR202506200003', 3, '京藏高速K50+200东', 'CAM003',
    '2025-06-20 08:00:00', 35.2000, 2, 28.6000, 2, 0.6200, 2, 15.80, 2,
    0, 0, 62.30, 3, 1, 'LOW_BRIGHTNESS,LOW_CONTRAST,OCCLUSION', 1,
    '画面偏暗、对比度低且存在轻微遮挡，建议检查', NOW()),
('VQR202506200004', 4, '京藏高速K50+200西', 'CAM004',
    '2025-06-20 08:00:00', 5.0000, 4, 2.1000, 2, 0.1500, 3, 0.00, 1,
    1, 0, 8.50, 5, 1, 'BLACK_SCREEN,BLUR', 1, '疑似黑屏，请立即排查', NOW());

INSERT INTO video_health_diagnosis (diagnosis_no, camera_id, camera_name, camera_code,
    road_name, longitude, latitude, diagnosis_date, period_type,
    total_detection_count, abnormal_count, normal_rate,
    avg_brightness, avg_contrast, avg_blur_score, avg_occlusion_ratio,
    avg_overall_score, min_overall_score, max_overall_score,
    health_score, health_level, health_level_label,
    black_screen_count, blur_count, occlusion_count, freeze_count,
    low_brightness_count, low_contrast_count, alert_count,
    online_rate, uptime_seconds, downtime_seconds,
    maintenance_status, recommendation, create_time) VALUES
('VHD202506200001', 1, '京港澳高速K100+500北', 'CAM001',
    '京港澳高速', 116.397400, 39.904200, '2025-06-20', 'DAILY',
    96, 0, 100.00,
    132.5000, 64.8000, 0.9150, 0.60,
    95.20, 92.50, 97.80,
    95.00, 1, '健康',
    0, 0, 0, 0,
    0, 0, 0,
    100.00, 86400, 0,
    0, '运行状态良好，无需维护', NOW()),
('VHD202506200002', 2, '京港澳高速K100+500南', 'CAM002',
    '京港澳高速', 116.397400, 39.903200, '2025-06-20', 'DAILY',
    96, 2, 97.92,
    118.2000, 59.5000, 0.8720, 2.80,
    91.50, 85.30, 96.80,
    90.00, 2, '亚健康',
    0, 0, 0, 0,
    2, 0, 0,
    99.50, 85968, 432,
    1, '早间偶有薄雾，建议关注晨雾时段画面', NOW()),
('VHD202506200003', 3, '京藏高速K50+200东', 'CAM003',
    '京藏高速', 116.407400, 39.914200, '2025-06-20', 'DAILY',
    96, 28, 70.83,
    52.3000, 35.6000, 0.6850, 12.50,
    72.80, 58.50, 88.20,
    65.00, 3, '异常',
    0, 5, 12, 0,
    18, 15, 4,
    92.50, 79920, 6480,
    2, '存在遮挡、亮度和对比度问题，请尽快安排现场检查', NOW()),
('VHD202506200004', 4, '京藏高速K50+200西', 'CAM004',
    '京藏高速', 116.408400, 39.914200, '2025-06-20', 'DAILY',
    96, 96, 0.00,
    12.5000, 8.2000, 0.1850, 0.20,
    25.30, 8.50, 42.80,
    15.00, 5, '故障',
    48, 24, 0, 12,
    36, 48, 24,
