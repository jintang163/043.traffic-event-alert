...
CREATE TABLE IF NOT EXISTS construction_plan (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    plan_code VARCHAR(64) NOT NULL UNIQUE COMMENT '施工计划编号',
    plan_name VARCHAR(256) NOT NULL COMMENT '施工计划名称',
    construction_type INT DEFAULT 1 COMMENT '施工类型：1-日常养护 2-道路维修 3-应急抢修 4-改扩建',
    construction_type_label VARCHAR(32) DEFAULT NULL COMMENT '施工类型标签',
    camera_id BIGINT COMMENT '关联摄像头ID',
    camera_name VARCHAR(128) COMMENT '摄像头名称',
    fence_id BIGINT COMMENT '关联施工区围栏ID(geo_fence.id)',
    buffer_fence_id BIGINT COMMENT '关联缓冲区围栏ID(geo_fence.id)',
    road_name VARCHAR(128) COMMENT '路段名称',
    location VARCHAR(512) COMMENT '施工位置描述',
    start_stake VARCHAR(32) COMMENT '起始桩号',
    end_stake VARCHAR(32) COMMENT '结束桩号',
    center_longitude DECIMAL(10,6) COMMENT '中心经度',
    center_latitude DECIMAL(10,6) COMMENT '中心纬度',
    polygon_points TEXT COMMENT '施工区多边形坐标JSON [[lng,lat],...]',
    polygon_points_pixel TEXT COMMENT '施工区像素坐标JSON [[nx,ny],...]',
    buffer_distance DECIMAL(8,2) DEFAULT 50.00 COMMENT '缓冲区距离 米',
    speed_limit DECIMAL(6,2) DEFAULT 60.00 COMMENT '施工区限速 km/h',
    standard_cone_count INT DEFAULT 0 COMMENT '标准锥桶数量',
    cone_spacing DECIMAL(6,2) DEFAULT 5.00 COMMENT '锥桶间距 米',
    plan_start_time DATETIME COMMENT '计划开始时间',
    plan_end_time DATETIME COMMENT '计划结束时间',
    actual_start_time DATETIME COMMENT '实际开始时间',
    actual_end_time DATETIME COMMENT '实际结束时间',
    construction_unit VARCHAR(256) COMMENT '施工单位',
    responsible_person VARCHAR(64) COMMENT '负责人',
    contact_phone VARCHAR(32) COMMENT '联系电话',
    worker_count INT DEFAULT 0 COMMENT '施工人员数量',
    equipment_info TEXT COMMENT '施工设备信息JSON',
    traffic_control_measures TEXT COMMENT '交通管控措施',
    plan_status INT DEFAULT 0 COMMENT '计划状态：0-草稿 1-待执行 2-进行中 3-已完成 4-已取消',
    plan_status_label VARCHAR(32) DEFAULT NULL COMMENT '计划状态标签',
    alert_enabled INT DEFAULT 1 COMMENT '是否启用告警',
    alert_level INT DEFAULT 2 COMMENT '告警级别 1-一般 2-严重 3-紧急',
    notify_enabled INT DEFAULT 1 COMMENT '是否启用通知',
    notify_dept_ids VARCHAR(512) COMMENT '通知部门ID列表，逗号分隔',
    link_work_order INT DEFAULT 1 COMMENT '是否自动联动工单',
    led_reminder_enabled INT DEFAULT 1 COMMENT '是否启用LED情报板提醒',
    led_default_message VARCHAR(256) DEFAULT '前方施工 减速慢行' COMMENT 'LED默认显示内容',
    event_count INT DEFAULT 0 COMMENT '关联事件数量',
    cone_alert_count INT DEFAULT 0 COMMENT '锥桶告警次数',
    intrusion_alert_count INT DEFAULT 0 COMMENT '闯入告警次数',
    speeding_alert_count INT DEFAULT 0 COMMENT '超速告警次数',
    ...
    INDEX idx_actual_time (actual_start_time, actual_end_time)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='施工计划表';

CREATE TABLE IF NOT EXISTS cone_detection_record (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    record_no VARCHAR(64) NOT NULL UNIQUE COMMENT '检测记录编号',
    plan_id BIGINT COMMENT '关联施工计划ID',
    plan_code VARCHAR(64) COMMENT '施工计划编号',
    plan_name VARCHAR(256) COMMENT '施工计划名称',
    camera_id BIGINT COMMENT '摄像头ID',
    camera_name VARCHAR(128) COMMENT '摄像头名称',
    detection_time DATETIME NOT NULL COMMENT '检测时间',
    detected_cone_count INT DEFAULT 0 COMMENT '检测到的锥桶数量',
    standard_cone_count INT DEFAULT 0 COMMENT '标准锥桶数量',
    missing_cone_count INT DEFAULT 0 COMMENT '缺失锥桶数量',
    extra_cone_count INT DEFAULT 0 COMMENT '多余锥桶数量',
    is_compliant INT DEFAULT 1 COMMENT '是否合规 0-不合规 1-合规',
    compliance_rate DECIMAL(5,2) DEFAULT 100.00 COMMENT '合规率 %',
    cone_positions TEXT COMMENT '锥桶位置JSON数组 [{x,y,confidence},...] 像素坐标',
    cone_positions_gis TEXT COMMENT '锥桶GIS坐标JSON数组 [{lng,lat,confidence},...]',
    avg_confidence DECIMAL(5,4) COMMENT '平均置信度',
    min_confidence DECIMAL(5,4) COMMENT '最低置信度',
    detection_algorithm VARCHAR(64) COMMENT '检测算法',
    snapshot_url VARCHAR(512) COMMENT '检测截图URL',
    heatmap_url VARCHAR(512) COMMENT '热力图URL',
    alert_triggered INT DEFAULT 0 COMMENT '是否触发告警 0-否 1-是',
    alert_level INT DEFAULT 1 COMMENT '告警级别',
    alert_id BIGINT COMMENT '关联告警事件ID',
    source_node_code VARCHAR(64) COMMENT '来源边缘节点编码',
    detection_result TEXT COMMENT '检测结果详情JSON',
    description VARCHAR(512) COMMENT '描述',
    create_time DATETIME,
    update_time DATETIME,
    deleted INT DEFAULT 0,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='锥桶检测记录表';

INSERT INTO construction_plan (plan_code, plan_name, construction_type, construction_type_label,
    camera_id, camera_name, fence_id, road_name, location,
    center_longitude, center_latitude, speed_limit, standard_cone_count, cone_spacing,
    plan_start_time, plan_end_time, construction_unit, responsible_person, contact_phone,
    worker_count, plan_status, plan_status_label, alert_enabled, alert_level,
    description, sort_order, create_time) VALUES
('CONS20250618001', '京港澳高速K100+300至K100+700路面养护', 1, '日常养护',
    1, '京港澳高速K100+500北', 1, '京港澳高速', '京港澳高速K100+300至K100+700北方向',
    116.397400, 39.904250, 60.00, 32, 5.00,
    '2025-06-20 08:00:00', '2025-06-20 18:00:00', '高速养护一队', '张队长', '13800138001',
    8, 2, '进行中', 1, 2,
    '日常路面养护作业，封闭最右侧车道', 1, NOW());

INSERT INTO cone_detection_record (record_no, plan_id, plan_code, plan_name,
    camera_id, camera_name, detection_time, detected_cone_count, standard_cone_count,
    missing_cone_count, is_compliant, compliance_rate, avg_confidence,
    alert_triggered, alert_level, description, create_time) VALUES
('CONE20250620001', 1, 'CONS20250618001', '京港澳高速K100+300至K100+700路面养护',
    1, '京港澳高速K100+500北', '2025-06-20 09:30:00', 32, 32,
    0, 1, 100.00, 0.9250,
    0, 1, '锥桶摆放整齐，数量符合标准', NOW()),
('CONE20250620002', 1, 'CONS20250618001', '京港澳高速K100+300至K100+700路面养护',
    1, '京港澳高速K100+500北', '2025-06-20 10:00:00', 28, 32,
    4, 0, 87.50, 0.9180,
    1, 2, '检测到4个锥桶缺失，请及时补放', NOW());

CREATE TABLE IF NOT EXISTS audio_event (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    event_no VARCHAR(64) NOT NULL UNIQUE COMMENT '事件编号',
    camera_id BIGINT COMMENT '摄像头ID',
    camera_name VARCHAR(128) COMMENT '摄像头名称',
    event_type VARCHAR(32) NOT NULL COMMENT '事件类型: HORN/COLLISION_SOUND/SIREN/ABNORMAL_NOISE',
    confidence DECIMAL(5,4) COMMENT '置信度',
    duration DECIMAL(8,2) COMMENT '持续时间(秒)',
    peak_db DECIMAL(8,2) COMMENT '峰值分贝',
    avg_db DECIMAL(8,2) COMMENT '平均分贝',
    dominant_freq DECIMAL(10,2) COMMENT '主频率(Hz)',
    event_time DATETIME NOT NULL COMMENT '事件发生时间',
    description VARCHAR(1024) COMMENT '描述',
    longitude DECIMAL(10,6) COMMENT '经度',
    latitude DECIMAL(10,6) COMMENT '纬度',
    location VARCHAR(512) COMMENT '位置描述',
    alert_status INT DEFAULT 0 COMMENT '处理状态 0-待处理 1-已处理 2-忽略',
    linked_alert_event_id BIGINT COMMENT '关联的告警事件ID',
    ambient_db DECIMAL(8,2) COMMENT '环境底噪分贝',
    audio_clip_url VARCHAR(512) COMMENT '音频片段URL',
    create_time DATETIME,
    update_time DATETIME,
    deleted INT DEFAULT 0,
    INDEX idx_event_no (event_no),
    ...
    INDEX idx_event_time (event_time),
    INDEX idx_alert_status (alert_status),
    INDEX idx_linked_alert_event_id (linked_alert_event_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='音频异常事件表';
