...
public class VideoQualityController {
    ...
    public Result<VideoQualityAnalysisResult> manualDetect(
        ...
        return Result.success(videoQualityService.manualDetect(cameraId, scenario));
    }
    ...
    public Result<Map<String, Object>> batchMockDetect(@RequestBody VideoQualityManualDetectRequest request) {
        ...
        if (cameraId != null) {
            VideoQualityAnalysisResult result = videoQualityService.manualDetect(cameraId, scenario);
            return Result.success(Map.of(
                    "total", 1,
                    "results", List.of(result)
            ));
        }
        return Result.success(Map.of("message", "请指定cameraId参数"));
    }
}
